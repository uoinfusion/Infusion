...
namespace Infusion.IO
{
    ...
    internal sealed class ClientNewGamePushStream : IPushStream
    {
        private readonly NewGameCrypt crypt;
        private readonly byte[] encrypted = new byte[1];
        private readonly byte[] tmp = new byte[1];

        public IPushStream BaseStream { get; set; }

        public ClientNewGamePushStream()
        {
        }

        public ClientNewGamePushStream(byte[] cryptoKey)
        {
            crypt = new NewGameCrypt(cryptoKey);
        }
    ...
    internal sealed class ClientNewGamePullStream : IPullStream
    {
        ...
        public int Read(byte[] buffer, int offset, int count)
        {
            if (crypt != null)
            {
                ...
                crypt.Encrypt(encrypted, buffer, count);
        ...
        public int ReadByte()
        {
            if (crypt != null)
            {
                ...
                crypt.Encrypt(encrypted, buffer, 1);
